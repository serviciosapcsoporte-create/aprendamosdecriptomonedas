...
export const Route = createFileRoute("/nivel-3/$topicSlug")({
  component: TopicPage,
  head: ({ params }) => {
    const topic = findTopic(params.topicSlug as string, "3");
    if (!topic) return { meta: [{ title: "Tema no encontrado" }] };
    return {
      meta: [
        { title: `${topic.title} | NIVEL 3 | Aprendamos de Criptomonedas` },
